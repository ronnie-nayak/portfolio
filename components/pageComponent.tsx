...
export function PageComponent(props: {
  ...
  secondImage: string;
  secondImageSize: string;
  thirdImage: string;
  thirdImageSize: string;
  builtWith: string[];
  website: string;
  github: string;
}) {
  ...
  return (
    <>
      <div
        id={props.id}
        className={`p-10 pt-24 flex flex-col items-center w-screen ${pageScroll ? "h-full" : "h-screen"} overflow-hidden overscroll-none`}
      >
        <div>
          <motion.div
          ...
          >
            <div className="w-full">
              ...
              <div className="flex flex-col sm:flex-row gap-4 m-8 ml-0">
                <a target="_blank" href={props.website} className="linker">
                  <h2>Visit Website</h2>
                  <TbWorld size={45} />
                  <LuArrowUpRight size={45} />
                </a>
                <a target="_blank" href={props.github} className="linker">
                  <h2>Github Code</h2>
                  <FaGithub size={45} />
                  <LuArrowUpRight size={45} />
                </a>
            ...
            <p className="w-full sm:text-[1.5vw] mb-2">{props.description}</p>
          </motion.div>
        ...
        <div>
          <motion.div
            className="w-[90vw] sm:w-[70vw] h-[60vw] sm:h-[40vw]  rounded-3xl mb-14 "
            style={{
        ...
        <div className="flex flex-col sm:flex-row gap-14">
          <motion.div
            className="w-[90vw] sm:w-[35vw] h-[60vw] sm:h-[40vw] rounded-3xl "
            style={{
        ...
        <div className="fixed left-0 top-0 z-20 w-screen h-screen ">
          <div className="pt-20 xl:w-[1300px] w-3/4 mx-auto">
            <div className="p-9 pt-40 flex gap-9">
              {data.map((item, index) => (
                <motion.div
                  layoutId={index + "project"}
                  className="w-[30%] h-[60vh] overflow-hidden translate-y-52"
                  key={index}
                  onLayoutAnimationComplete={() =>
                    router.push("./?project=true")
                  }
                ></motion.div>
              ))}
            </div>
      ...
      {first && (
        <motion.div
          className="fixed top-0 left-0 p-24 grid place-items-center w-screen h-screen bg-cream z-10 overscroll-y-contain"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
